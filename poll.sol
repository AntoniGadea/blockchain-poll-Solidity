...
contract PollContract {
    ...
    modifier pollExists(uint64 _pollId) {
    ...
    function getPoll(uint64 _pollId) external view  pollExists(_pollId) returns(uint256, string memory, string memory, uint64[] memory, bytes32[] memory) {
        return (
            polls[_pollId].id,
    ...
    function vote(uint64 _pollId, uint64 _vote) external pollExists(_pollId) {
